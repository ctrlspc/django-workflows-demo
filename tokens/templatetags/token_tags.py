...
class CanMakeTransitionNode(template.Node):
    ...
    def handle_token(cls, parser, token):
        ...
        try:
            tag_name, object_under_test , transition = token.split_contents()
        except ValueError:
            raise template.TemplateSyntaxError("%r tag requires exactly two arguments" % tag_name)
        
        
        end_tag = 'end_can_make_transition'
        
        nodelist_true = parser.parse(('else', end_tag)) # parse everythin between the strt tag and an else (if it exists) or the end tag
        ...
        return cls(object_under_test , transition, nodelist_true, nodelist_false)
    
    
    def __init__(self, object_under_test, transition_string, nodelist_true, nodelist_false):
        self.object_under_Test = template.Variable(object_under_test)
        self.transition_string = transition_string
        self.nodelist_true = nodelist_true
        self.nodelist_false = nodelist_false
...
@register.tag(name='can_make_transition')
def do_can_make_transition(parser, token):
    return CanMakeTransitionNode.handle_token(parser, token)
